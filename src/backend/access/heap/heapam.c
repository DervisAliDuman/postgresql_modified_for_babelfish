...
continue_page:
		...
		for (; linesleft > 0; linesleft--, lineoff += dir)
		{
			...
			visible = HeapTupleSatisfiesVisibility(scan->rs_base.rs_rd,
												   tuple,
		   ...
		   bool keep_buf)
{
	 ...
	 */
	valid = HeapTupleSatisfiesVisibility(relation,
										 tuple,
										 snapshot,
										 buffer);
...
l1:
	...
	/* BABELFISH
 	* Additional parameter: relation is added when calling HeapTupleSatisfiesVisibility, 
	* which was introduced for Table Variables
 	*/
